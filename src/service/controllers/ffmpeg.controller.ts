...
class FFMPEGMANAGER {
  ...
  async ffmpeg(inputVideosSource: string, outputVideoSrc: string) {
    try {
      const inputVideosSrc = `${inputVideosSource}`;
  ...
  async processVideo(
    ...
    try {
      ...
      const outPutPath = await this.ffmpeg(inputVideosSource, outputVideoSrc);
      return outPutPath;
    } catch (error) {
      throw error;
    }
