...
class VIDEOFLOW {
  async executeProcess(inputVideosSource: string) {
    try {
      const context = {
      ...
      const videoSource = await videosourceController.createVideoSource(inputVideosSource, context);
      ...
      try {
        await ffmpegController.processVideo(inputVideosSource, videoSource.fileSources.tmp, context, videoSource._id);
      } catch (error) {
